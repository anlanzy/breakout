...
object NetGameHolder extends js.JSApp {
  ...
  override def main(): Unit = {
    ...
    //timeHandler = dom.window.setInterval(() => gameLoop(), Protocol.frameRate)

    //systemLoop()
    dom.window.setInterval(() => gameLoop(), Protocol.frameRate)
    //dom.window.cancelAnimationFrame(123)
  }

  //game loop
  /*  def systemLoop : () => Int = () => {
      gameLoop()
      timeHandler = dom.window.setTimeout(systemLoop, Protocol.frameRate)
      timeHandler
    }*/
    ...
    gameStream.onmessage = { (event: MessageEvent) =>
      ...
      wsMsg match {
        ...
        case a@Protocol.SnakeAction(id, keyCode, frame) =>
          ...
          var late = frame - grid.frameCount
          if (late > 0) {
            //dom.window.clearInterval(timeHandler)
            writeToArea(s"!!! got snake action=$a when i am in frame=${grid.frameCount}")
/*            while (late > 0) {
              writeToArea(s"frame=${grid.frameCount}, additional update needed: $late, update it.")
              grid.update()
              late -= 1
            }*/
            //systemLoop()
          } else if (late < 0) {
            writeToArea(s"!!!!!!!! got OLD snake action=$a when i am in frame=${grid.frameCount}")
          } else {
            writeToArea(s"got snake action=$a in right frame frame=${grid.frameCount} ")
          }
        ...
        case data: Protocol.GridDataSync =>
          writeToArea(s"Grid data FULL SYNC")
